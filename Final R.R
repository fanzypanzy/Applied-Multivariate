...
data$BL_YADB5 <- droplevels(data$BL_YADB5) ##This line gives error
...
########################### ANALYSIS ################################################
cdata <- read.csv("road.csv")
cdata <- cdata[,-1] ##Need to remove the X column in the imputed file Gillian gave
names(cdata)
...
ggplot(melted,aes(x=variable, y=value, fill=variable)) + geom_boxplot() +
  xlab("Item") + theme(legend.position="none") + ggtitle("Boxplots for Answers to All Questions")

## There is an error with the graph below, think its with the braketing but I dont know
## the ggplot package to fix it, sorry :(
